...
#developed by NVIDIA for GPU-accelerated computing

def train(ch, options, train_options, beam_id):#Setting CUDA Device
    with torch.cuda.device(options['gpu_idx']):#function begins by specifying the GPU device to be used
        ...
        options['multi_step'] = options['multi_step'].cuda() #these variables are moved to
        options['ph_table'] = options['ph_table'].cuda()     #GPU for computation efficiency

        real_time_perf = np.zeros((train_options['num_iter'],))  # storing performance metrics for plotting
        real_time_perf_n = np.zeros((train_options['num_iter'],))  #storing performance metrics for plotting

        actor_net = Actor(options['num_ant'], options['num_ant'])
        critic_net = Critic(2 * options['num_ant'], 1, ch, options['H_r'], options['H_i'])
        #five arguments
        #2 * options['num_ant']: Input dimension #1: Output dimension, single value
        #ch: Input data representing the channel information
        ounoise = OUNoise((1, options['num_ant'])) #Ornstein-Uhlenbeck noise process, which is often used for exploration in DDPG.
        env = envCB(ch, options['num_ant'], options['num_bits'], beam_id, options)
#envCB object plays a crucial role in RL training by defining the environment dynamics,
#facilitating interaction between the agent and the environment, computing rewards, managing state transitions, 
        actor_net.cuda() #actor_net model to the GPU memory. 
        critic_net.cuda()
        ...
        # -------------- training -------------- #
        replay_memory = train_options['replay_memory'] #agent stores experiences
        iteration = 0 #keeps track of the current iteration
        num_of_iter = train_options['num_iter'] #iteration until it reaches the total number of iterations
        while iteration < num_of_iter:

            # Proto-action
            actor_net.eval() #inference (making predictions or generating outputs) rather than training.
            action_pred = actor_net(state)
 #Predicts an action (beamforming vector) based on the current state (state) using the actor network.
#action represents the agent's decision on how to adjust the signal transmission.
           
            reward_pred, bf_gain_pred, action_quant_pred, state_1_pred = CB_Env.get_reward(action_pred)
#Passes the predicted action to the environment (CB_Env) to obtain the predicted rewards etc            
            # reward_pred = torch.from_numpy(reward_pred).float().cuda()

            critic_net.eval() #evaluation mode,  only using it for inference.
            q_pred = critic_net(state, action_quant_pred)
#Predicts the Q-value (expected future reward) based on the current state and the quantized action
            real_time_perf[iteration] = bf_gain_pred
#Records the predicted beamforming gain at the current iteration

            # Exploration and Quantization Processing
            
            action_pred_noisy = ounoise.get_action(action_pred, t=train_options['overall_iter']) 
            #Adds noise to the predicted action to encourage exploration and prevent the agent from getting stuck in suboptimal solutions.
...
#action_quant: Quantizes the noisy action using a phase table to obtain a discrete action that can be applied in the environment.            
...
#Records the actual beamforming gain (bf_gain) at the current iteration for performance comparison with the predicted gain.

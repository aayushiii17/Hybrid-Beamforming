...
class Critic_(nn.Module):  # inherits from the nn.Module class provided by the PyTorch library, definition of a neural network model.

    def __init__(self, input_size, output_size): #self (a reference to the current instance of the class), input: state, output: action
        super(Critic_, self).__init__() #calls the constructor of the parent class (nn.Module)

        self.scaling_factor = 16 #scale the size of the hidden layers in the neural network.
        self.fc1 = nn.Linear(input_size, self.scaling_factor * input_size) #linear transformation, 
        self.bn1 = nn.BatchNorm1d(self.scaling_factor * input_size) #created for the first output layer,stabilize and accelerate teh traning 
...
#Phase to Beamforming Conversion:

        x_state = phase2bf(state)
        x_state_r, x_state_i = x_state[:, :self.M], x_state[:, self.M:]
#Beamforming Operation:
        z_r = x_state_r @ self.H_r + x_state_i @ self.H_i #zr​= xstater ​​⋅Hr ​+ xstatei ​​⋅Hi​
        z_i = x_state_r @ self.H_i - x_state_i @ self.H_r

        z = z_r ** 2 + z_i ** 2 #z= ∣zr​∣2 + ∣zi​∣2
        z_min = torch.mean(z, dim=1).reshape(-1, 1) #zmin ​= mean(z)
#Action Processing:
        x_action = phase2bf(action) # 𝑥 action = phase2bf ( action ) 
        x_action_r, x_action_i = x_action[:, :self.M], x_action[:, self.M:] #xactionr​​, xactioni​​=Real(xaction​),Imag(xaction​)
#Second Beamforming Operation: 
        u_r = x_action_r @ self.H_r + x_action_i @ self.H_i #𝑢 𝑟 = 𝑥 action 𝑟 ⋅ H 𝑟 + 𝑥 action 𝑖 ⋅ H 𝑖  ​
        u_i = x_action_r @ self.H_i - x_action_i @ self.H_r

        u = u_r ** 2 + u_i ** 2 # u = ∣ur​∣2 + ∣ui​∣2
